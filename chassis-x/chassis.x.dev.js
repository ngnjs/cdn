/**
  * v1.0.38 generated on: Thu Aug 11 2016 19:43:48 GMT+0000 (UTC)
...
if (!NGN) {
  ...
  class Driver {
     ...
     * The callback receives an array of valid HTMLElements that can be modified or
    ...
    render (name, data, parent, position, callback) {
      ...
      NGN.NET.template(this.templates[name], data, (elements) => {
        if (NGN.hasOwnProperty('DOM')) {
          elements.forEach((element) => {
            NGN.DOM.svg.update(element, () => {
              this.adjustedRender(parent, elements, position, callback)
            })
          })
        } else {
          this.adjustedRender(parent, elements, position, callback)
    ...
    adjustedRender (parent, elements, position, callback) {
      elements.forEach((element) => {
