/**
  * v1.0.98 generated on: Sat Jul 09 2016 20:46:33 GMT+0000 (UTC)
...
Object.defineProperties(NGN, {
  ...
  stack: NGN.get(function () {
    ...
    stack = stack.filter(function (item) {
      ...
      item = item.replace(/^.*\s\(/i, '').replace(/\)/gi, '').replace(/^.*\@/i, '').replace(me.nodelike ? process.cwd() : window.location.origin, '').replace(/\s{1,100}at\s{1,100}/gi, '').trim().split(':');
